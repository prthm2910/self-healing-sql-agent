...
def fix_and_test():
    ...
    # Define index fields to cover both memories and lessons
    index_config = {
    ...
    print(f"🛠️  Initializing PostgresStore with fields: {index_config['fields']}")
    store = PostgresStore(pool, index=index_config)
    
    try:
        store.setup()
        print("✅ Store setup successful.")
    except Exception as e:
